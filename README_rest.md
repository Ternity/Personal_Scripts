...
- 生成输入卡之后，与程序手册上的关键词进行对比，做如下确认：
  ...
  - 需要明确出现在输入卡的关键词有：
    ...
    2. 计算体系相关关键词: `spin`, `charge`, `spin_polarization`等
...
- `eri_type`: 取值String类型。自洽场运算中的四中心积分计算方法，目前REST支持：
  ...
  2. `ri-v`: 全称为resolution of identity，又名density fitting，是对四中心积分进行张量分解后的近似算法。(缺省)
...
- `initial_guess`: 取值String。分子体系进行自洽场运算所用的初始猜测方法。目前REST支持:
  ...
  2. `vsap`: Superposition of Atomic Potentials的初始猜测方法。采用半经验方法对体系势能项进行估计，与libcint生成的动能项进行加和后得到初始的Fock矩阵
  3. `hcore`: Hcore则对应单电子近似初猜，直接将由libcint生成的hcore矩阵作为初始猜测的fock矩阵进行计算
